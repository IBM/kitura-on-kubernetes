...
UBUNTU_VERSION=$(echo "$version" | awk -F. '{print $1"."$2}')
UBUNTU_VERSION_NO_DOTS=$(echo "$version" | awk -F. '{print $1$2}')
...
wget https://swift.org/builds/"$SNAPSHOT_TYPE"/"$UBUNTU_VERSION_NO_DOTS"/"$SWIFT_SNAPSHOT"/"$SWIFT_SNAPSHOT"-"$UBUNTU_VERSION".tar.gz \
  ...
  && sudo cp -RT "$SWIFT_SNAPSHOT"-"$UBUNTU_VERSION"/usr/ /usr/local/ \
  && rm "$SWIFT_SNAPSHOT"-"$UBUNTU_VERSION".tar.gz \
  && sudo chmod -R go+r /usr/local/lib/swift \
  && swift --version
